...
const statusOptions = [
  { value: 'online', label: 'Driving', Icon: Truck, color: 'emerald' },
  { value: 'idle', label: 'Idling', Icon: Pause, color: 'amber' },
  { value: 'on_break', label: 'On Break', Icon: Coffee, color: 'blue' },
  { value: 'repair', label: 'Repair', Icon: Wrench, color: 'red' },
];
...
export default function DriverPage() {
  ...
  const statusInfo = statusOptions.find((s) => s.value === driverStatus) || statusOptions[0];
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 mb-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {statusOptions.map((opt) => {
                const Icon = opt.Icon;
                const isActive = driverStatus === opt.value;
                return (
                  <button
                    key={opt.value}
                    ...
                    className={
                      ...
                      (isActive
                        ? 'bg-' + opt.color + '-500/10 border-' + opt.color + '-500/50'
                        : 'bg-slate-700/50 border-slate-700 hover:border-slate-600')
                    }
                  >
                    <Icon className={'w-4 h-4 mb-1.5 ' + (isActive ? 'text-' + opt.color + '-400' : 'text-slate-500')} />
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-2xl p-8 mb-5 text-center">
          ...
          {isTracking && (
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <span className="text-sm text-' + statusInfo.color + '-400 font-medium">{statusInfo.label}</span>
            </div>
          )}
          {lastUpdate && (
                ...
                return (
                  <button
                    ...
                    className={
                      ...
                      (isActive
                        ? 'bg-' + opt.color + '-500/10 border border-' + opt.color + '-500/50'
                        : 'bg-slate-700/50 border border-slate-700 hover:border-slate-600')
                    }
                  >
                    <Icon className={'w-4 h-4 mx-auto mb-1 ' + (isActive ? 'text-' + opt.color + '-400' : 'text-slate-500')} />
...
console.log('Driver page written');
